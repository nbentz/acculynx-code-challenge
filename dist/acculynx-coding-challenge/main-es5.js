...
var QuestionsComponent = /** @class */ (function () {
    function QuestionsComponent(questionsService, dialog) {
    ...
    QuestionsComponent.prototype.openDialog = function (question) {
        var _this = this;
        var dialogRef = this.dialog.open(_results_dialog_results_dialog_component__WEBPACK_IMPORTED_MODULE_4__["ResultsDialogComponent"], {
            data: question
        });
        dialogRef.afterClosed().subscribe(function (result) {
            _this.show = false;
            _this.showGuesses = false;
            _this.questionsService.score += result.totalScore;
        });
    };
    ...
    QuestionsComponent.prototype.scorePointsAndUpdate = function (question, answer) {
        this.questions = this.questionsService.questions;
        !answer.guess_count ? answer.guess_count = 1 : answer.guess_count += 1;
        var indexAnswer = question.answers.findIndex(function (x) { return x.answer_id === answer.answer_id; });
        question.answers[indexAnswer] = answer;
        question.timeStamp = new Date().getTime();
        var indexQuestion = this.questions.findIndex(function (x) { return x.question_id === question.question_id; });
        this.questions[indexQuestion] = question;
        this.guessedQuestions = this.questionsService.guessedQuestions;
        var indexGuessedQuestion = this.guessedQuestions.findIndex(function (x) { return x.question_id === question.question_id; });
        indexGuessedQuestion >= 0
            ? this.guessedQuestions[indexGuessedQuestion] = question
            : this.guessedQuestions = [question].concat(this.guessedQuestions);
...
var QuestionsService = /** @class */ (function () {
    ...
    */
    QuestionsService.prototype.getAllQuestions = function () {
        var _this = this;
        var questionsResult = this.http.get("https://api.stackexchange.com/2.2/search/advanced?pagesize=10&order=desc&sort=creation&accepted=True&answers=2&site=stackoverflow&filter=!)rh-4Rv3X0MXqJcCydZX");
        var guessedQuestionsResult = this.http.get("/questions/guessed");
        Object(rxjs__WEBPACK_IMPORTED_MODULE_3__["forkJoin"])([questionsResult, guessedQuestionsResult]).subscribe(function (results) {
            _this.questions = results[0].items;
            _this.questions = _this.shuffleNestedArrays(_this.questions);
            // let qArr = results[0].items.$.questions;
            // qArr = this.shuffle(qArr);
            _this.guessedQuestions = results[1];
            _this.guessedQuestions = _this.shuffleNestedArrays(_this.guessedQuestions);
            _this.updateLocalQuestions();
            return results;
        });
    };
    // updates a question with current question values.
    QuestionsService.prototype.updateQuestion = function (question) {
        return this.http.patch("/questions/update", question).subscribe(function (data) { });
    };
    // updates local questions array with current question values.
    QuestionsService.prototype.updateLocalQuestions = function () {
        var _this = this;
        this.guessedQuestions.forEach(function (g) {
            _this.questions.forEach(function (q) {
                if (q.question_id === g.question_id) {
                    var index = _this.questions.findIndex(function (x) { return x.question_id === g.question_id; });
                    _this.questions[index] = g;
                }
            });
        });
        return;
    ...
    QuestionsService.prototype.shuffleNestedArrays = function (array) {
        var _this = this;
        array.forEach(function (x, i) {
            array[i].answers = _this.shuffle(array[i].answers);
        });
        return array;
    };
    // shuffles items in array.
    QuestionsService.prototype.shuffle = function (array) {
        var _a;
...
var ResultsDialogComponent = /** @class */ (function () {
    ...
    ResultsDialogComponent.prototype.formatData = function () {
        var score = 0;
        var correctScore = '';
        var guessScore = '';
        var scoreScore = '';
    ...
    ResultsDialogComponent.prototype.closeDialog = function () {
